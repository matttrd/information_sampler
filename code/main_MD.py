...
def cfg():
    ...
    pilot = True # if True, pilot net mode (min dataset exp) and save indices according to their weight
...
def get_counter_sorting():
    '''
    Get sorted index (descending=False)
    '''
    sorted_v, sorted_idx = torch.sort(ctx.count, descending=False)
...
def main():
    ...
    if ctx.opt['pilot']:
        pilots = dict()
        pilots['pilot_directory'] = ctx.opt['filename']
        pilots['pilot_saved'] = ctx.opt['save']
        ...
        pilot_fn = '_'.join(('pilot_', ctx.opt['dataset'], ctx.opt['arch'], ctx.opt['sampler'], str(ctx.opt['temperature'])))
